...
async function filtering(myRange, departure, arrival, startDate, endDate) {
    try {
        const response = await fetch(`http://localhost:3000/tours?category_id=1&price_lt=${myRange}&departure_location=${departure}&arrival_location=${arrival}&start_date=${startDate}&end_date=${endDate}`);
        if (!response.ok) {
