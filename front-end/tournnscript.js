...
async function filtering(myRange, departure, arrival, startDate, endDate) {
    try {
        const response = await fetch(`http://localhost:10000/tours?category_id=2&price_lte=${myRange}&departure_location=${departure}&arrival_location=${arrival}&start_date=${startDate}&end_date=${endDate}`);
        if (!response.ok) {
...
async function fetchTours() {
    try {
        const response = await fetch('http://localhost:10000/tours?category_id=2');
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
